                            ...
                            <div class="product-img-box col-lg-4 col-sm-5 col-xs-12">
                                <div class="product-image">
                                    <div class="product-full">
                                        <img id="product-zoom" src="{{(@$agro->image)?asset('public/backend/images/Agro_images/'.$agro->image):asset('public/backend/images/Agro_images/noimage.jpg')}}" data-zoom-image="{{(@$agro->image)?asset('public/backend/images/Agro_images/'.$agro->image):asset('public/backend/images/Agro_images/noimage.jpg')}}" alt="product-image"/> </div>
                                </div>
                                <!-- end: more-images -->
                            </div>
                            <div class="product-shop col-lg-8 col-sm-7 col-xs-12">
                                <div class="product-name">
                                    <h1>Name: {{$agro->name}}</h1>
                                </div>
                                <div class="rating"><span>Type: {{$agro['agro']['name']}}</span></div>
                                <div class="price-block">
                                    <div class="price-box">
                                        <p class="special-price"> <span class="price-label">Special Price</span><span class="price">Special Price: {{(@$agro->special_prise)?$agro->special_prise:""}}</span></p>
                                        <p class="old-price"> <span class="price-label">Regular Price:</span><span class="price">Regular Price: {{(@$agro->regular_prise)?$agro->regular_prise:""}}</span></p>
                                        <div class="rating"><span>Discount Prise: {{(@$agro->discount_prise)?$agro->discount_prise:""}}</span></div>
                                        <br>
                                        <div class="rating"><span>Bulk Prise: {{(@$agro->bulk_prise)?$agro->bulk_prise:""}}</span></div>
                                        <br>
                                        ...
                                        <div class="rating"><span>Size: {{(@$agro->size)?$agro->size:""}}</span></div>
                                        <div class="rating"><span>Color: {{(@$agro->color)?$agro->color:""}}</span></div>
                                        <div class="rating"><span>Cultivation Time: {{(@$agro->cultivation_time)?$agro->cultivation_time:""}}</span></div>
                                        <div class="rating"><span>Harvesting Time: {{(@$agro->harvesting_time)?$agro->harvesting_time:""}}</span></div>
                                    </div>
                                </div>
                                <div class="short-description">
                                    <h2>Product Description</h2>
                                    <p>{{$agro->description}}</p>
                                </div>
                            </div>
                        </form>
                        @endif
                                ...
                                <div class="blog-inner">
                                    @foreach($agro_products as $agro)
                                        ...
                                        <div class="col-xs-12 col-sm-4 blog-preview_item">
                                            <div class="entry-thumb jtv-blog-img-hover"> <a href="{{route('AgroProducts.view')}}"> <img alt="Blog" src="{{(@$agro->image)?asset('public/backend/images/Agro_images/'.$agro->image):asset('public/backend/images/Agro_images/noimage.jpg')}}"> </a> </div>
                                            <h4 class="blog-preview_title"><a href="{{route('AgroProducts.view')}}">{{$agro->name}}</a></h4>
                                            <div class="blog-preview_info">
                                                <ul class="post-meta">
                                                    <li><a href="{{route('AgroProducts.view')}}">{{$agro['agro']['name']}}</a></li>
