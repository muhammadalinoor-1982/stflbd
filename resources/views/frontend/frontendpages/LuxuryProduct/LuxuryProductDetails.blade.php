                            ...
                            <div class="product-img-box col-lg-4 col-sm-5 col-xs-12">
                                <div class="product-image">
                                    <div class="product-full">
                                        <img id="product-zoom" src="{{(@$luxury->image)?asset('public/backend/images/Luxury_images/'.$luxury->image):asset('public/backend/images/Luxury_images/noimage.jpg')}}" data-zoom-image="{{(@$luxury->image)?asset('public/backend/images/Luxury_images/'.$luxury->image):asset('public/backend/images/Luxury_images/noimage.jpg')}}" alt="product-image"/> </div>
                                </div>
                                <!-- end: more-images -->
                            </div>
                            <div class="product-shop col-lg-8 col-sm-7 col-xs-12">
                                <div class="product-name">
                                    <h1>Name: {{$luxury->name}}</h1>
                                </div>
                                <div class="rating"><span>Type: {{$luxury['luxury']['name']}}</span></div>
                                <div class="price-block">
                                    <div class="price-box">
                                        <p class="special-price"> <span class="price-label">Special Price</span><span class="price">Special Price: {{(@$luxury->special_prise)?$luxury->special_prise:""}}</span></p>
                                        <p class="old-price"> <span class="price-label">Regular Price:</span><span class="price">Regular Price: {{(@$luxury->regular_prise)?$luxury->regular_prise:""}}</span></p>
                                        <div class="rating"><span>Discount Prise: {{(@$luxury->discount_prise)?$luxury->discount_prise:""}}</span></div>
                                        <br>
                                        <div class="rating"><span>Bulk Prise: {{(@$luxury->bulk_prise)?$luxury->bulk_prise:""}}</span></div>
                                        <br>
                                        <p class="availability in-stock">
                                            @if($luxury->quantity != 0)
                                            <span>In Stock</span>
                                            @else
                                                <span style="background-color: red">out of Stock</span>
                                            @endif
                                        </p>
                                        <div class="rating"><span>Size: {{(@$luxury->size)?$luxury->size:""}}</span></div>
                                        <div class="rating"><span>Color: {{(@$luxury->color)?$luxury->color:""}}</span></div>
                                        <div class="rating"><span>Production Lead Time: {{(@$luxury->production_lead_time)?$luxury->production_lead_time:""}}</span></div>
                                        <div class="rating"><span>Delivery Lead Time: {{(@$luxury->delivery_lead_time)?$luxury->delivery_lead_time:""}}</span></div>
                                    </div>
                                </div>
                                ...
                                <div class="blog-inner">
                                    @foreach($luxury_products as $luxury)
                                        ...
                                        <div class="col-xs-12 col-sm-4 blog-preview_item">
                                            <div class="entry-thumb jtv-blog-img-hover"> <a href="{{route('LuxuryProducts.view')}}"> <img alt="Blog" src="{{(@$luxury->image)?asset('public/backend/images/Luxury_images/'.$luxury->image):asset('public/backend/images/Luxury_images/noimage.jpg')}}"> </a> </div>
                                            <h4 class="blog-preview_title"><a href="{{route('LuxuryProducts.view')}}">{{$luxury->name}}</a></h4>
                                            <div class="blog-preview_info">
                                                <ul class="post-meta">
                                                    <li><a href="{{route('LuxuryProducts.view')}}">{{$luxury['luxury']['name']}}</a></li>
